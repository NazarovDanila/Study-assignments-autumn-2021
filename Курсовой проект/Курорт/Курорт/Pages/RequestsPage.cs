...
namespace Курорт.Pages
{
    public partial class RequestsPage : Form
    {
        ...
        private void RequestsPage_Load(object sender, EventArgs e)
        {
            ...
            requestsBindingSource.DataSource = db.Requests.ToList();
        }
        //КНОПКИ
        private void AddDataButton_Click(object sender, EventArgs e)
        {
            WorkWithRequestsDataPage page = new WorkWithRequestsDataPage();//Создаем объект формы
            page.db = db;
            page.req = null;//Передаем в WorkWithDataPage значение null для свойства usr
            ...
            if (dr == DialogResult.OK)//Если пользователь DialogResut == OK
            {
                ...
                requestsBindingSource.DataSource = null;
                requestsBindingSource.DataSource = db.Requests.ToList();
            }
        }
        private void ChangeDataButton_Click(object sender, EventArgs e)
        {
            Requests req = (Requests)requestsBindingSource.Current;//Получаем текущий объект (на него указывается в DataGridView)
            WorkWithRequestsDataPage page = new WorkWithRequestsDataPage();//Создаем форму для работы с данными            
            page.db = db;
            page.req = req;//Передаем в форму объект 
        ...
        private void DelDataButton_Click(object sender, EventArgs e)
        {
            ...
            DialogResult dr = MessageBox.Show("Вы действительно хотите удалить запись о запросе - " + req.RequestID.ToString()
            ...
            {
                db.Requests.Remove(req);//Удаляем выбранный объект из коллекции
                //Сохраняем сделанные изменения в БД
                try//Обрабатываем исключения
                {
                    db.SaveChanges();//Сохраняем сделанные изменения в БД                    
                    requestsBindingSource.DataSource = db.Requests.ToList();//Обновляем содержание userBindingSource
                }
